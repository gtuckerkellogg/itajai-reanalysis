...
## the main model is stop at 0.1, 0.5 % probability, always stop on hospitalisation
## The alt model is to just stop on infection
...
overview_gt_to_latex <- function(gt_obj,model_name,probabilistic,infected_only)  {
    caption <- sprintf("\\caption{%s %s %s}",
                       ...
                       sprintf("Statistics for hospitalisations and deaths are %s",
        ...
        str_replace_all(c("[$]"="",
                          "longtable"="tabulary",
                          "0.000"="<0.001")) |>
            str_replace(fixed("\\begin{tabulary}{l|rrrrrrrrrrrr}"),
                        "\n\n\\begin{table}[h!tb]\n\\begin{fullwidth}\\footnotesize\n\\begin{tabulary}{\\linewidth}{lRrrRRrrRRrrR}") |>
            str_replace(fixed("\\begin{tabulary}{l|rrrrrrrr}"),
                        "\n\n\\begin{table}[h!tb]\n\\begin{fullwidth}\\footnotesize\n\\begin{tabulary}{\\linewidth}{lRrrRRrrR}") |>
                paste0(sprintf("%s\n\\end{fullwidth}\\end{table}\n\n",caption),collapse="\n")
}

### Start

supplemental_tables <- tryCatch({
    load(here::here('results',"supplemental_tables.RData"));
    supplemental_tables
},
error=function(cond) {
...
for (dataset in names(model_names)) {
    supplemental_tables[[dataset]] <- sprintf("\n\n\\clearpage\n\\subsection{%s model simulations}\n\n",model_names[[dataset]])    
    if (!exists(dataset)) {
        gc()
        print(sprintf("loading %s",dataset))
        load(here('results',sprintf("%s.RData",dataset)))
        gc()
    }    

    for (probabilistic in c(TRUE,FALSE)) {
        print('reassigning regularity')
        cohorts <- reassign_regularity(eval(sym(dataset)),probabilistic=probabilistic)
        for (infected_only in c(TRUE,FALSE)) {

            sim_name <- sprintf("%s-%s-%s",
                                dataset,
                                ifelse(infected_only,"infected","all"),
                                ifelse(probabilistic,"probabilistic","uniform"))
            print(sprintf("Generating supplemental table for %s with %s individuals and %s stop on infection",
                          dataset,
                          ifelse(infected_only,"infected","all"),
                          ifelse(probabilistic,"probabilistic","uniform")))
            
            overview_dfs[[sim_name]] <-overview_outcomes(cohorts,infected_only=infected_only)
            supplemental_tables[[sim_name]] <- overview_gt(overview_dfs[[sim_name]]) |>
                cols_hide(columns=matches("corrected")) |>
                overview_gt_to_latex(model_names[[dataset]],probabilistic,infected_only)
                
        }
    }
   save(supplemental_tables,file=here::here('results',"supplemental_tables.RData"))
}
